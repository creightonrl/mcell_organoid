---
title: "M cell organoid digital PCR analysis"
author: "Rachel Creighton"
date: "2026-07-15"
output:
  html_document:
    keep_md: true
---
```{r setup, message=FALSE, warning=FALSE, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
library(tidyverse)
library(here)
```

# Overview
In this project, we used digital PCR to measure the expression of M cell marker genes (GP2, SpiB, CCL20), interferon-stimulated genes (ISG15, MX1, IFI6, etc), and TLRs. 

We initially iterated through several previously published organoid media formulations to determine which resulted in the highest M cell marker expression. These experiments were performed in duodenal-derived organoids from a single donor.

# M cell marker expression
## Base media comparison
The first set of experiments, evaluating "differentiation" media published by Ding et al and "patterning" and "maturation" media published by He et al were repeated 3 times.

```{r get data, message=FALSE}
experiments <- paste("organoid5", "organoid6", "organoid7", sep = "|")
file_names <- list.files(path = here("data", "dpcr", "raw"),
                         pattern = experiments)

data <- list()
for (i in 1:length(file_names)) {
  tmp <- read_csv(here("data", "dpcr", "raw", file_names[i])) %>%
    mutate(experiment_name = str_extract(file_names[i], "^\\d{4}_\\d{4}_\\w{9}")) %>%
    rename(chamber = "ChamberID",
          sample_desc = "SampleName",
          droplet_num = "TotalNumberOfDroplets",
          blue_conc = "Blue_Channel_Concentration",
          green_conc = "Green_Channel_Concentration",
          experiment_name = "experiment_name") %>%
    mutate(chamber = str_sub(chamber, -2)) %>%
    select(chamber, sample_desc, droplet_num, blue_conc, green_conc, experiment_name) %>%
    pivot_longer(c(blue_conc, green_conc), names_to = "color", names_pattern = "(.*)_conc", values_to = "concentration")
  
  data[[i]] <- tmp
}

tidy_data <- bind_rows(data)
```

```{r get pcr metadata, message=FALSE}
file_names <- list.files(path = here("data", "dpcr", "metadata"),
                         pattern = experiments)

metadata <- list()
for (i in 1:length(file_names)) {
  tmp <- readxl::read_xlsx(here("data", "dpcr", "metadata", file_names[i])) %>%
    mutate(experiment_name = str_extract(file_names[i], "^\\d{4}_\\d{4}_\\w{9}")) %>%
    mutate(sample = as.character(sample)) %>%
    pivot_longer(c(blue, green), names_to = "color", values_to = "gene")
  
  metadata[[i]] <- tmp
}

tidy_metadata <- bind_rows(metadata)
```

```{r get organoid experiment metadata, message=FALSE}
file_names <- list.files(path = here("data", "metadata"),
                         pattern = experiments)

organoid_metadata <- list()
for (i in 1:length(file_names)) {
  tmp <- read_csv(here("data", "metadata", file_names[i])) %>%
    mutate(organoid_experiment = str_extract(file_names[i], "^\\w{9}")) %>%
    mutate(sample = as.character(sample))
  
  organoid_metadata[[i]] <- tmp
}

tidy_organoid_metadata <- bind_rows(organoid_metadata)
```

```{r combine data and metadata, message=FALSE}
tidy_full_data <- full_join(tidy_data, tidy_metadata) %>%
  filter(droplet_num >= 8000) %>%
  filter_out(sample == "NTC" | sample == "NRT") %>%
  drop_na() %>%
  mutate(orig_concentration = concentration * dilution_factor) %>%
  mutate(organoid_experiment = str_extract(experiment_name, "organoid\\d+$"))
```

```{r normalize data, message=FALSE}
normalized_data <- tidy_full_data %>%
  dplyr::select(c(organoid_experiment, sample, orig_concentration, gene)) %>%
  filter_out(orig_concentration == "Inf") %>%
  pivot_wider(names_from = gene, values_from = orig_concentration, values_fn = mean) %>%
  mutate(across(.cols = c("CCL20", "GP2", "SPIB"),
                .fns = ~ .x / RPP30,
                .names = "{.col}_norm"))

normalized_data <- full_join(normalized_data, tidy_organoid_metadata) %>%
  mutate(condition = paste(media, mcell_factors, sep = "_"))
```

```{r plot data, fig.height=3, fig.width=5, warning=FALSE}
mean_data <- normalized_data %>%
  group_by(organoid_experiment, media, mcell_factors, condition) %>%
  summarize(
    across(
      ends_with("_norm"),       
      ~ mean(.x, na.rm = TRUE),
      .names = "{.col}_mean"),
    .groups = "drop_last") %>%
  mutate(GP2_norm_mean = GP2_norm_mean + 0.0001) %>%
  mutate(SPIB_norm_mean = SPIB_norm_mean + 0.0001)

plot_data <- normalized_data %>%
  select(organoid_experiment, sample, CCL20_norm, GP2_norm, SPIB_norm, media, mcell_factors, condition) %>%
  pivot_longer(c(CCL20_norm, GP2_norm, SPIB_norm), names_to = "gene", values_to = "norm_concentration") %>%
  mutate(detectable = if_else(norm_concentration == 0, "undetectable", "detectable", "no data")) %>%
  mutate(norm_concentration = norm_concentration + 0.0001)

ggplot(filter(plot_data, gene == "GP2_norm"), aes(x = condition, y = norm_concentration, color = organoid_experiment)) +
    geom_jitter(width = 0.1) +
    geom_point(data = mean_data, 
               aes(x = condition, y = GP2_norm_mean), 
               size = 6, 
               shape = "\u2014") +
    scale_y_log10(breaks = c(0.0001, 0.001, 0.01, 0.1, 1), labels = c(0, 0.001, 0.01, 0.1, 1)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("GP2/RPP30")

ggplot(filter(plot_data, gene == "SPIB_norm"), aes(x = condition, y = norm_concentration, color = organoid_experiment)) +
    geom_jitter(width = 0.1) +
    geom_point(data = mean_data, 
               aes(x = condition, y = SPIB_norm_mean), 
               size = 6, 
               shape = "\u2014") +
    scale_y_log10(breaks = c(0.0001, 0.01, 0.1, 1, 10, 100), labels = c(0, 0.01, 0.1, 1, 10, 100)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("SPIB/RPP30")

ggplot(filter(plot_data, gene == "CCL20_norm"), aes(x = condition, y = norm_concentration, color = organoid_experiment)) +
    geom_jitter(width = 0.1) +
    geom_point(data = mean_data, 
               aes(x = condition, y = CCL20_norm_mean), 
               size = 6, 
               shape = "\u2014") +
    scale_y_log10(limits = c(1, 1000)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("CCL20/RPP30")
```

Alternatively, data can be plotted with media formulation on the x axis and colored based on the inclusion of M cell factors. Here, the data points represent the mean of technical replicates from three independent experiments.  
```{r additional plot options, fig.height=3, fig.width=4}
ggplot(mean_data, aes(x = media, y = GP2_norm_mean, color = mcell_factors)) +
    geom_jitter(width = 0.1) +
    stat_summary(fun = mean, geom = "point", shape = "\u2014", size = 6) +
    scale_y_log10(breaks = c(0.0001, 0.001, 0.01, 0.1, 1), labels = c(0, 0.001, 0.01, 0.1, 1)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("GP2/RPP30")

ggplot(mean_data, aes(x = media, y = SPIB_norm_mean, color = mcell_factors)) +
    geom_jitter(width = 0.1) +
    stat_summary(fun = mean, geom = "point", shape = "\u2014", size = 6) +
    scale_y_log10(breaks = c(0.0001, 0.01, 0.1, 1, 10, 100), labels = c(0, 0.01, 0.1, 1, 10, 100)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("SPIB/RPP30")

ggplot(mean_data, aes(x = media, y = CCL20_norm_mean, color = mcell_factors)) +
    geom_jitter(width = 0.1) +
    stat_summary(fun = mean, geom = "point", shape = "\u2014", size = 6) +
    scale_y_log10(limits = c(1, 1000)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("CCL20/RPP30")
```


We prioritized patterning media in future experiments because it gave the most consistent increases in GP2 and SPIB expression.

## Lymphotoxin wash
### Experiments 8 and 9
```{r, message=FALSE}
experiments <- paste("organoid8", "organoid9", sep = "|")
file_names <- list.files(path = here("data", "dpcr", "raw"),
                         pattern = experiments)
data <- list()
for (i in 1:length(file_names)) {
  tmp <- read_csv(here("data", "dpcr", "raw", file_names[i])) %>%
    mutate(experiment_name = str_extract(file_names[i], "^\\d{4}_\\d{4}_\\w{9}")) %>%
    rename(chamber = "ChamberID",
          sample_desc = "SampleName",
          droplet_num = "TotalNumberOfDroplets",
          blue_conc = "Blue_Channel_Concentration",
          green_conc = "Green_Channel_Concentration",
          experiment_name = "experiment_name") %>%
    mutate(chamber = str_sub(chamber, -2)) %>%
    select(chamber, sample_desc, droplet_num, blue_conc, green_conc, experiment_name) %>%
    pivot_longer(c(blue_conc, green_conc), names_to = "color", names_pattern = "(.*)_conc", values_to = "concentration")
  data[[i]] <- tmp
}

tidy_data <- bind_rows(data)
```

```{r, message=FALSE}
file_names <- list.files(path = here("data", "dpcr", "metadata"),
                         pattern = experiments)
metadata <- list()
for (i in 1:length(file_names)) {
  tmp <- readxl::read_xlsx(here("data", "dpcr", "metadata", file_names[i])) %>%
    mutate(experiment_name = str_extract(file_names[i], "^\\d{4}_\\d{4}_\\w{9}")) %>%
    mutate(sample = as.character(sample)) %>%
    pivot_longer(c(blue, green), names_to = "color", values_to = "gene")
  metadata[[i]] <- tmp
}

tidy_metadata <- bind_rows(metadata)
```

```{r, message=FALSE}
file_names <- list.files(path = here("data", "metadata"),
                         pattern = experiments)
organoid_metadata <- list()
for (i in 1:length(file_names)) {
  tmp <- read_csv(here("data", "metadata", file_names[i])) %>%
    mutate(organoid_experiment = str_extract(file_names[i], "^\\w{9}")) %>%
    mutate(sample = as.character(sample))
  organoid_metadata[[i]] <- tmp
}

tidy_organoid_metadata <- bind_rows(organoid_metadata)
```

```{r, message=FALSE}
tidy_full_data <- full_join(tidy_data, tidy_metadata) %>%
  filter(droplet_num >= 8000) %>%
  filter_out(sample == "NTC" | sample == "NRT") %>%
  drop_na() %>%
  mutate(orig_concentration = concentration * dilution_factor) %>%
  mutate(organoid_experiment = str_extract(experiment_name, "organoid\\d+$"))

normalized_data <- tidy_full_data %>%
  dplyr::select(c(organoid_experiment, sample, orig_concentration, gene)) %>%
  filter_out(orig_concentration == "Inf") %>%
  pivot_wider(names_from = gene, values_from = orig_concentration, values_fn = mean) %>%
  mutate(across(.cols = c("CCL20", "GP2", "SPIB"),
                .fns = ~ .x / RPP30,
                .names = "{.col}_norm"))

normalized_data <- full_join(normalized_data, tidy_organoid_metadata) 
```

```{r LT wash plots, fig.height=2.5, fig.width=3.5, warning=FALSE}

ggplot(normalized_data, aes(x = mcell_factors, y = GP2_norm)) +
    geom_jitter(width = 0.1, aes(color = donor)) +
    stat_summary(fun = mean, geom = "point", shape = "\u2014", size = 6) +
    scale_y_log10(breaks = c(0.0001, 0.001, 0.01, 0.1, 1), labels = c(0, 0.001, 0.01, 0.1, 1)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("GP2/RPP30")

ggplot(normalized_data, aes(x = mcell_factors, y = SPIB_norm)) +
    geom_jitter(width = 0.1, aes(color = donor)) +
    stat_summary(fun = mean, geom = "point", shape = "\u2014", size = 6) +
    scale_y_log10(breaks = c(0.0001, 0.01, 0.1, 1, 10, 100), labels = c(0, 0.01, 0.1, 1, 10, 100)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("SPIB/RPP30")

ggplot(normalized_data, aes(x = mcell_factors, y = CCL20_norm)) +
    geom_jitter(width = 0.1, aes(color = donor)) +
    stat_summary(fun = mean, geom = "point", shape = "\u2014", size = 6) +
    scale_y_log10(limits = c(1, 1000)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("CCL20/RPP30")
```

M cell markers are expressed following differentiation with LTa2b1 in 4 different donors, and the M cell marker expression is maintained even when the differentiation media is replaced with differentiation media without LTa2b1 for the last 48 hours. This is important for the M cell project because LTa2b1 strongly reactivates HIV in J-Lat and U1 cell lines.  

## Alternatives to LTa2b1
### Experiments 10 and 11
```{r, message=FALSE}
experiments <- paste("organoid10", "organoid11", sep = "|")
file_names <- list.files(path = here("data", "dpcr", "raw"),
                         pattern = experiments)
data <- list()
for (i in 1:length(file_names)) {
  tmp <- read_csv(here("data", "dpcr", "raw", file_names[i])) %>%
    mutate(experiment_name = str_extract(file_names[i], "^\\d{4}_\\d{4}_\\w{10}")) %>%
    rename(chamber = "ChamberID",
          sample_desc = "SampleName",
          droplet_num = "TotalNumberOfDroplets",
          blue_conc = "Blue_Channel_Concentration",
          green_conc = "Green_Channel_Concentration",
          experiment_name = "experiment_name") %>%
    mutate(chamber = str_sub(chamber, -2)) %>%
    select(chamber, sample_desc, droplet_num, blue_conc, green_conc, experiment_name) %>%
    pivot_longer(c(blue_conc, green_conc), names_to = "color", names_pattern = "(.*)_conc", values_to = "concentration")
  data[[i]] <- tmp
}

tidy_data <- bind_rows(data)
```

```{r, message=FALSE}
file_names <- list.files(path = here("data", "dpcr", "metadata"),
                         pattern = experiments)
metadata <- list()
for (i in 1:length(file_names)) {
  tmp <- readxl::read_xlsx(here("data", "dpcr", "metadata", file_names[i])) %>%
    mutate(experiment_name = str_extract(file_names[i], "^\\d{4}_\\d{4}_\\w{10}")) %>%
    mutate(sample = as.character(sample)) %>%
    pivot_longer(c(blue, green), names_to = "color", values_to = "gene")
  metadata[[i]] <- tmp
}

tidy_metadata <- bind_rows(metadata)
```

```{r, message=FALSE}
file_names <- list.files(path = here("data", "metadata"),
                         pattern = experiments)
organoid_metadata <- list()
for (i in 1:length(file_names)) {
  tmp <- read_csv(here("data", "metadata", file_names[i])) %>%
    mutate(organoid_experiment = str_extract(file_names[i], "^\\w{10}")) %>%
    mutate(sample = as.character(sample))
  organoid_metadata[[i]] <- tmp
}

tidy_organoid_metadata <- bind_rows(organoid_metadata)
```

```{r, message=FALSE}
tidy_full_data <- full_join(tidy_data, tidy_metadata) %>%
  filter(droplet_num >= 8000) %>%
  filter_out(sample == "NTC" | sample == "NRT" | gene == "TLR4") %>%
  drop_na() %>%
  mutate(orig_concentration = concentration * dilution_factor) %>%
  mutate(organoid_experiment = str_extract(experiment_name, "organoid\\d+$"))

normalized_data <- tidy_full_data %>%
  dplyr::select(c(organoid_experiment, sample, orig_concentration, gene)) %>%
  filter_out(orig_concentration == "Inf") %>%
  pivot_wider(names_from = gene, values_from = orig_concentration, values_fn = mean) %>%
  mutate(across(.cols = c("CCL20", "GP2", "SPIB"),
                .fns = ~ .x / RPP30,
                .names = "{.col}_norm"))

normalized_data <- full_join(normalized_data, tidy_organoid_metadata, by = ) 
```

```{r alt LT plots, fig.height=2.5, fig.width=3.5, warning=FALSE}

ggplot(normalized_data, aes(x = mcell_factors, y = GP2_norm)) +
    geom_jitter(width = 0.1, aes(color = donor)) +
    stat_summary(fun = mean, geom = "point", shape = "\u2014", size = 6) +
    scale_y_log10(breaks = c(0.0001, 0.001, 0.01, 0.1, 1), labels = c(0, 0.001, 0.01, 0.1, 1)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("GP2/RPP30")

ggplot(normalized_data, aes(x = mcell_factors, y = SPIB_norm)) +
    geom_jitter(width = 0.1, aes(color = donor)) +
    stat_summary(fun = mean, geom = "point", shape = "\u2014", size = 6) +
    scale_y_log10(breaks = c(0.0001, 0.01, 0.1, 1, 10, 100), labels = c(0, 0.01, 0.1, 1, 10, 100)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("SPIB/RPP30")

ggplot(normalized_data, aes(x = mcell_factors, y = CCL20_norm)) +
    geom_jitter(width = 0.1, aes(color = donor)) +
    stat_summary(fun = mean, geom = "point", shape = "\u2014", size = 6) +
    scale_y_log10(limits = c(1, 1000)) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    theme_classic() +
    ylab("CCL20/RPP30")
```

Alternative lymphotoxin beta receptor agonists result in similar M cell marker expression compared to LTa2b1, which could act through the TNF receptor or the lymphotoxin beta receptor. LTa1b2 was prioritized in following experiments because it gave similar M cell marker expression as LTa2b1, but did not reactivate HIV in J-Lat or U1.  

## Organoid stimulation
### Experiments 12 and 13